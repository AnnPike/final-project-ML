import numpy as np
import logging
from sklearn import tree, linear_model, ensemble, model_selection
...
class WaveletsForestRegressor:
  def __init__(self, regressor='cv_random_forest', criterion='mse', bagging=0.8, depth=9, trees=5, features='auto', seed=None):
    '''
  ...
  def fit(self, X_raw, y):
    ...
    X_raw = (X_raw - np.min(X_raw, 0))/(np.max(X_raw, 0) - np.min(X_raw, 0))
    ...
    #   rf = regressor.fit(X_raw, y)
    # elif self.regressor == 'cv_random_forest':
    #   param_grid = {'max_depth': [10, 15, 20, None]}
    #   estimator = ensemble.RandomForestRegressor(bootstrap=True, criterion='mse',
    #                                              max_features='sqrt', warm_start=True, n_estimators=150)
    #   regressor = model_selection.GridSearchCV(estimator=estimator, cv=5, param_grid=param_grid,
    #                                            n_jobs=-1, verbose=1)
  ...
  def evaluate_smoothness(self, m=1000):
    ...
    for m_step in range(2, m, step):
      ...
      MSE_error = MSE(self.y, predictions)
